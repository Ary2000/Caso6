...
Bird.prototype.init_mesh = function(position, orientation) {
	...
	var material =  new THREE.MeshBasicMaterial( { color: 0x00000, side: THREE.DoubleSide } )
...
Bird.prototype.update_position = function(elapsed_time, lookAt) {
	...
	var velocidadActual = this.velocity.length();
	this.distanciaTotal += displacement.length();
	var colorActual = 0x000000;

	//Seccion CopyPaste

	if(this.distanciaTotal > 2.55746){
		colorActual = colorActual + 0x9966CC;
	  }
	  if(this.distanciaTotal > 2.58271){
		colorActual = colorActual + 0xFF00FF;
	  }
	  if(this.distanciaTotal < 2.49727){
		colorActual = colorActual + 0x33CC66;
	  }
	  if(this.distanciaTotal < 1.9376){
		colorActual = colorActual + 0x00FF00;
	  }
	  if(velocidadActual > 4.25417){
		colorActual = colorActual + 0xFF9966;
	  }
	  if(velocidadActual > 4.31716){
		colorActual = colorActual + 0xFF0000;
	  }
	  if(velocidadActual < 3.80757){
		colorActual = colorActual + 0x00CCFF;
	  }
	  if(velocidadActual < 2.12287){
		colorActual = colorActual + 0x0000FF;
	  }

	  //Seccion CopyPaste

	this.mesh.material = new THREE.MeshBasicMaterial( { color:colorActual, side: THREE.DoubleSide } );
